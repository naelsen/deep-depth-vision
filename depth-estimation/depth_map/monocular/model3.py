...
def create_model():
    ...
    down = False
    i = 0
    for N in [8, 16, 32, 64, 128, 256, 128, 64, 32, 16, 8]:
        if N == 256:
            down = True
        if not down:
            i += 1
    ...
    x = keras.layers.Conv2D(1, kernel_size=(3, 3), activation=activation, padding='same')(x)
...
def create_model_bis():
    ...
    down = False
    i = 0
    tab = np.array([64, 128, 256, 512, 256, 128, 64])
    tab = np.array([4, 8, 16, 32, 64, 128, 64, 32, 16, 8, 4])
    for N in tab:
        if N == np.max(tab):
            down = True
        if not down:
            ...
            if N == np.min(tab):
                if N >= 64:
                    x = keras.layers.SeparableConv2D(N ,kernel_size=(3, 3), activation=activation, padding="same")(input)
                else:
                    x = keras.layers.Conv2D(N ,kernel_size=(3, 3), activation=activation, padding="same")(input)
            else:
                if N >= 64:
            ...
            if N >= 64:
                ...
                x = keras.layers.Conv2D(N, kernel_size=(3, 3), activation=activation, padding="same")(x)
            if N == np.min(tab):
                x_out = keras.layers.Dropout(DROP_INPUT)(x)
            else:
                x_out = keras.layers.Dropout(DROP)(x)
            x_out = MP(x_out)
            x_list.append((x, x_out))
        else:
            if N == np.max(tab) and N >= 64:
                x = keras.layers.SeparableConv2D(N, kernel_size=(3, 3), activation=activation, padding="same")(x_list[-1][1])
                x = keras.layers.SeparableConv2D(N, kernel_size=(3, 3), activation=activation, padding="same")(x)
            elif N == np.max(tab) and N < 64:
                x = keras.layers.Conv2D(N, kernel_size=(3, 3), activation=activation, padding="same")(x_list[-1][1])
                x = keras.layers.Conv2D(N, kernel_size=(3, 3), activation=activation, padding="same")(x)
            else:
                i -= 1
...
model = create_model_bis()
#model.summary()
model.load_weights("weight512.h5")
